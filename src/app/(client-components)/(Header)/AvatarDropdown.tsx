...
export default function AvatarDropdown({ className = "", isOpen, onToggle, dropdownRef }: Props) {
  ...
  useEffect(() => {
    if (localStorage.getItem("user")) {
      const user: any | undefined | null = AuthService.authUser(redirectToLogin);
      if (
        user &&
        user!.user !== undefined &&
        user!.user!.profile !== undefined &&
        user!.user!.profile!.first_name !== undefined &&
        user!.user!.profile!.country !== undefined &&
        user!.user!.profile!.country.name !== undefined &&
        user!.user!.email !== undefined
      ) {
        setName(user.user.profile.first_name + " " + user.user.profile.last_name);
  ...
  return (
    <>
      <Popover className={`AvatarDropdown relative flex ${className}`} ref={dropdownRef}>
        <Popover.Button
              ...
              <div className="relative grid grid-cols-1 gap-6 bg-white dark:bg-neutral-800 py-7 px-6">
                <div className="flex items-center space-x-3">
                  ...
                  <div className="flex-grow w-12">
                    ...
                    <p className="text-xs mt-0.5">{country}</p>
                  </div>
                </div>
                ...
                >
                  ...
                  <div className="ml-4">
                    <p className="text-sm font-medium">{t("myTickets")}</p>
                  </div>
                </Link>
